...
# This module implements integration tests for the hybrid search logic.
# We populate our OpenSearch and Qdrant database engines with test chunks,
# then query them using lexical BM25, semantic vector, and fused hybrid search modes.

from __future__ import annotations # Allow self-referencing type annotations.
from unittest.mock import MagicMock, patch # Standard library mocking tools.
...
from src.retrieval.hybrid_search import hybrid_search # Unified search under test.
from src.shared.models import Chunk, ChunkingStrategy, FilingType # Data contracts.

# Declare this as an integration test requiring Docker services.
...
@patch("src.retrieval.vector_search.SECEmbedder") # Mock the embedder to avoid real API/model downloads.
async def test_hybrid_search_scenarios(mock_embedder_class: MagicMock) -> None:
    ...
    """
    # ── Step 1: Mock the Embedder ──
    mock_embedder = MagicMock()
    mock_embedder_class.return_value = mock_embedder
    
    # Generate mock vectors matching the dimension.
    dim = 1536
    vector_a = [0.0] * dim
    vector_a[0] = 1.0 # Unique unit direction.
    
    vector_b = [0.0] * dim
    vector_b[1] = 1.0 # Orthogonal unit direction.
    
    # Configure mock embedder to return vector_a for the query search.
    mock_embedder.embed.return_value = [vector_a]

    # ── Step 2: Initialize managers and clean state ──
    ...
    # Refresh OpenSearch to make index segments searchable.
    os_manager.client.indices.refresh(index=os_manager.index_name)

    # ── Step 4: Run BM25 search scenario ──
    # Query for "operating margins" (should match chunk_a).
    ...
    # ── Step 5: Run Vector search scenario ──
    # The mock embedder will return vector_a, which matches chunk_a.
    ...
    # ── Step 6: Run Hybrid Search (RRF) scenario ──
    # Run hybrid search which gathers and fuses results.
    ...
    # Top result should be chunk_a because it matches both semantically and lexically.
